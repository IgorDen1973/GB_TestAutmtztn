...
public class CustomLogger implements WebDriverEventListener {
    // имплементирует интерфейс-слушатель, а мы переопределяем отдельные, нужные нам, методы

    private static Logger logger = LoggerFactory.getLogger(WebDriver.class);


    @Override  // перед кликом на элемент выводим его свойства в лог(String log): TagName, Enabled, Displayed
    ...
    @Override  // при ошибке создает файл failure-screenshot-*.png  в папке /target  и закидывает в него скриншот ошибки
    ...
    }  // ScreenshotMaker - нами созданный класс с методом makeScreenshot()
