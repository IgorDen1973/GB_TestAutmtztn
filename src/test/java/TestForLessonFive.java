...
public class TestForLessonFive {
    ...
    @BeforeAll
    public static void setupWebDriverManager() {  // настраиваем браузер
    ...
    public void beforeTest() {
        setUpDriverSession();
        login();                   // сначала логинимся на страницу
    }
    ...
    public void createNewProject() {
        ...
        // После того , как залогинимся по BeforeEach, проверка на все элементы меню первой страницы:
        navigationTabsVisibilityTest();

        // Использование Actions для наведения мыши на пункт меню.
        By btnLocator = By.xpath(            // создаем локатор кнопки "Проекты" - объект класса "By"
                "//*[@id='main-menu']/ul/li[3]/a/span");

        Actions actions = new Actions(driver);  // создаем объект класса Actions
        // далее ищем и создаем объект в меню
        WebElement menuBtn = driver.findElement(btnLocator);
        actions.moveToElement(menuBtn);  // перемещаемся на элемент меню ("Панели инструментов")

        // Сабменюшка "Мои проекты"
        ...
        // Кликаем на менюшку "Организация" и совершаем выбор (названия меняются постянно)
    ...
    public void createNewContactPerson() {
        ...
        // Ввод в поле "Имя"
        driver.findElement(By.xpath("//div[1]/div[2]/div[2]/input[@name='crm_contact[firstName]']")).sendKeys("Рафаэль");
        ...
        driver.findElement(By.xpath("//div[2]/div[2]/input[@name='crm_contact[jobTitle]']")).sendKeys("Звездочет");
    ...
    public void createNewOrganisation() {
        ...
        driver.findElement(By.xpath("//div[1]/div[1]/div[2]/input[@name='crm_company[name]']")).sendKeys("Tesla.Com");

        // Ввод в поле "Краткое название"
        driver.findElement(By.xpath("//div[1]/div[2]/div[2]/input[@name='crm_company[shortName]']")).sendKeys("TSL");

        // Кликаем на менюшку "Форма собственности" и выбираем "ОАО"
        ...
        // Кликаем на менюшку "Статус" и выбираем "Активный"
        ...
        // Кликаем на менюшку "Отрасль" и выбираем "Машиностроение"
        ...
        driver.findElement(By.xpath("//div[2]/input[@name='crm_company[addresses][1][street]']")).sendKeys("Большая Полянка");
        driver.findElement(By.xpath("//div[2]/input[@name='crm_company[addresses][1][building]']")).sendKeys("20к4");
        ...
        driver.findElement(By.xpath("//div/div[1]/input[@name='crm_company[phones][1][prefixCode]']")).sendKeys("925");
        driver.findElement(By.xpath("//div/div/div[1]/input[@name='crm_company[phones][1][phone]']")).sendKeys("654");
        driver.findElement(By.xpath("//div/div/div[1]/input[@name='crm_company[phones][1][internalCode]']")).sendKeys("9087");

        // Ввод данных в поле "Контактная информация" E-mail
        driver.findElement(By.xpath("//div[2]/div[2]/div[2]/input[@name='crm_company[email]']")).sendKeys("info@tesla.com");
    ...
    public void createHouseHoldAgreement() {
        ...
        driver.findElement(By.xpath("//div[2]/input[@name='crm_domcontract[description]']"))
                .sendKeys("Срочные работы по ликвидации последствий снегопада в Анголе");
        ...
        if(visibleAlert){
            ...
            driver.findElement(By.xpath("//div[2]/input[@name='crm_domcontract[name]']")).sendKeys("Договор на уборку снега в июле"+x);
            // Нажимаем "Сохранить и закрыть"
            driver.findElement(By.cssSelector("button[class='btn btn-success action-button']")).click();
            // Поиск всплывающего сообщения-подтверждения, добавление Assert
            WebElement element = driver.findElement(By.xpath("//div[@class='alert alert-success fade in top-messages ']"));
    ...
    private void login() {                           // метод авторизации
        ...
        loginButton.click();
        //  Ассертим , что залогинились корректно
    ...
    private void setUpDriverSession() {
        ...
        options.setPageLoadStrategy(PageLoadStrategy.EAGER);  // загрузка интерактивная
        driver.manage().timeouts().implicitlyWait(5, TimeUnit.SECONDS); // ставим задержки
    }

    public void navigationTabsVisibilityTest() {
        ArrayList<String> mainMenuXpathSelectors =  // создаем список xpath-ов необходимых элементов страницы
                new ArrayList(Arrays.asList(
                        ...
                        ".//a/span[@class='title' and i[@class='icon-gear']]", // xpath для "Система"
                        ".//span[@class='title' and text()='Помощь']"
                ));
        // проверяем для каждого элемента (по его xpath) , что он обнаружен на странице:
